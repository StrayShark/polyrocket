...
- **v2.62** (2026-06-22) — v0.104: coverage round 13 (ModelLab promoteMut: 3 thresholds crossed) + codegen Phase 4 batch 10 (8 commands: sidecar + train + markets + wallet)。**v0.104a** (1 commit, +192 lines): 新 test 文件 `ModelLab.round13.test.tsx` (6 tests, 3 real + 3 placeholder) — 3 promoteMut tests 实跑 (success → 3 query invalidations + toast.success, error response → toast.error, throws → toast.error);3 promoteAllTrials placeholder tests 跳过 (需要 train:finished event listener 完整 mock)。**Project totals**: stmts 88.97→89.36 (+0.39pp, +11 stmts) ✓ stmts 89% crossed, br 86.19→86.35 (+0.16pp, +4 br), fn 83.87→84.59 (+0.72pp, +7 fn) ✓ fn 84% crossed, lines 90.04→90.35 (+0.31pp, +8 lines)。**3 thresholds crossed in one round** — stmts 88→89, fn 82→84, lines 89→90。ModelLab.tsx stmts 73.64→~80 (+6pp),fn 57.44→~62。**v0.104b** (1 commit, +324 lines): codegen batch 10 — 8 commands — `train_job` (TrainResultCodegen w/ nested TrainTrialDtoCodegen + `params: String` placeholder for serde_json::Value);`sync_markets` + `recompute_signals` (returns u32 row count);`sidecar_health_now` + `sidecar_health_snapshot` (SidecarHealthSnapshotCodegen w/ 5 i64 BigInt fields + nested SidecarHealthRowCodegen + enum SidecarHealthKindCodegen);`start_sidecar` + `stop_sidecar` (SidecarStatusCodegen);`polyrocket_wallet_set_pk` (WalletSetPkArgsCodegen)。**Codegen 64→72 = 64% of 112 IPCs**。**i64 handling**: timestamps 全部用 `#[specta(type = BigInt)]` (at_ms / success_count / failure_count / last_success_at_ms / last_failure_at_ms);count fields 用 i32 placeholder (duration_ms / trial_index)。**Complex DTO**: TrainResultCodegen 内嵌 Vec<TrainTrialDtoCodegen> + `best_params: Option<String>` (real impl is Option<serde_json::Value>, drift detect only);SidecarHealthSnapshotCodegen 内嵌 Vec<SidecarHealthRowCodegen>。**New pattern (v0.104b)**: enum types in specta (SidecarHealthKindCodegen as unit variant enum)。**u64 → u32 workaround**: `TrainJobArgsCodegen.timeout_ms: Option<u32>` (real impl is Option<u64>) — BigInt-forbidden on Option<u64>。Test count 1058→1064 (+6)。详细见 `polyrocket-v0.104-final.md` ship log。
- **v2.63** (2026-06-22) — v0.105: coverage round 14 (PromoteHistory rollback + selection: 2 thresholds crossed) + codegen Phase 4 batch 11 (6 commands: audit + promote + backtest)。**v0.105a** (1 commit, +214 lines): 新 test 文件 `PromoteHistory.round14.test.tsx` (5 tests, 4 real + 1 placeholder) — 3 rollback tests (success + rolled_back=true 3 invalidations, rolled_back=false toast.error, throws onError);1 selection test (stateful wrapper to test 3-item limit + drop oldest);1 placeholder (deselect timing — happy-dom controlled checkbox 不稳)。**Project totals**: stmts 89.36→89.92 (+0.56pp, +16 stmts) — stmts 90% 还差 0.08pp (~2 stmts), br 86.35→86.66 (+0.31pp, +8 br) — br 87% 还差 0.34pp (~9 br), **fn 84.59→85.01 (+0.42pp, +4 fn) ✓ fn 85% crossed**, **lines 90.35→91.05 (+0.70pp, +17 lines) — lines 91% well over 90% threshold**。PromoteHistory.tsx stmts 59.01→~78 (+19pp),br ~70→~85。**v0.105b** (1 commit, +367 lines): codegen batch 11 — 6 commands — `audit_count_for_actor` (returns u32);`rollback_model` (RollbackModelArgsCodegen → RollbackResultCodegen w/ BigInt timestamp);`auto_promote_if_better` (AutoPromoteIfBetterArgsCodegen → AutoPromoteIfBetterResultCodegen w/ BigInt timestamp);`backtest_model` (BacktestModelArgsCodegen w/ nested Vec<BacktestSampleCodegen> → BacktestResultCodegen w/ Vec<Vec<f64>> calibration buckets);`promote_model` (PromoteModelArgsCodegen → PromoteModelResultCodegen);`promote_all_trials` (PromoteAllTrialsResultCodegen w/ nested Vec<PromoteTrialResultCodegen>) — 6 commands 是 ModelLab 的核心,coverage 终于到 promote/rollback/backtest。**Codegen 72→78 = 70% of 112 IPCs**。**i64 handling**: timestamps `rolled_back_at_ms` / `promoted_at_ms` 用 BigInt,count 字段 `trial_index` / `sample_count` / `n_winners` / `n_losers` 用 i32 placeholder。**Complex DTO**: BacktestResultCodegen w/ nested `Vec<Vec<f64>>` calibration (5 buckets × N values) + `Vec<BacktestSampleCodegen>`;PromoteAllTrialsResultCodegen w/ nested `Vec<PromoteTrialResultCodegen>`。**New pattern (v0.105b)**: Vec<Vec<T>> nested in args/return — specta 2.0.0-rc.25 handles this fine。Test count 1064→1069 (+5)。详细见 `polyrocket-v0.105-final.md` ship log。
- **v2.64** (2026-06-22) — v0.110: 优先支持国产大模型最新版本。2 commits (main + final): **v0.110 main** (commit `80aa42c`): 加 5 个 Tier 1 国产 OpenAI 兼容 provider (Qwen / Doubao / Kimi / GLM / MiniMax) 到 `/welcome` LlmStep,5 match arms in `src-tauri/src/commands/llm.rs:765-772` (映射 5 个新 provider id → `ProviderKind::OpenaiCompat`) + duplicate 5 match arms in `src-tauri/src/infra/scheduler/mod.rs:578-580`。**Zero new Rust client** (CustomClient(OpenaiCompat) 已经能处理所有 OpenAI 兼容协议)。**v0.110-final** (commit `45d168c`): coding-spec v2.23 entry + ship log。**Doc**: new `docs/llm-providers.md` (5.5KB, 10 provider matrix + Tier 2 路线)。**Default model IDs (v0.110 placeholder, replaced in v0.110.2)**: Qwen `Qwen3-Max` / Doubao `Doubao-1.5-Pro` / Kimi `kimi-k2` / GLM `GLM-4.5` / MiniMax `MiniMax-Text-01`。**Coverage**: 89.92/86.66/85.01/91.05 (unchanged)。**Test count**: 1069→1072 (+3 LlmStep tests)。详细见 `polyrocket-v0.110-final.md` ship log。
- **v2.65** (2026-06-22) — v0.110.2: LlmStep `default_model` 落库 + 国产 provider latest-model ID 落地。1 commit。**Key insight**: v0.110 的 `defaultModel` 字段只写在 `PROVIDERS` 数组 (UI 初始值),LlmStep `onAdd` 没把 `default_model` 写进 SQLite,Rust dispatch 层会回退到 `LlmProviderDto::default()` → 用户实际用的 model ID 是过时的占位符。**Fix**: LlmStep `onAdd` 头部加 `llmProviderUpsert` 调用 with 5-piece `UpsertLlmProviderArgs`: `{ id, display_name, kind, api_base: defaultBase || undefined, default_model: defaultModel, enabled: true, timeout_ms: 30000, max_retries: 2 }`。**5 provider `kind` mapping**: openai→`openai`, anthropic→`anthropic`, google→`google`, deepseek→`deepseek`, qwen/doubao/kimi/glm/MiniMax/custom→`openai_compat` (5 match arms 已经在 `commands/llm.rs` from v0.110)。**5 latest model IDs (web_search 2026-06-22)**: Qwen `qwen3.7-max` / Doubao `doubao-seed-2-0-pro-260215` / Kimi `kimi-k2.7-code` / GLM `glm-5.2` / MiniMax `MiniMax-M2.7`。**Tests**: LlmStep.test.tsx 11→16 (+5 tests, each asserts `default_model` arg passed to `llmProviderUpsert` for one of 5 Chinese providers, plus 1 test that asserts openai default-model is `gpt-4o`)。**Doc updates**: `docs/llm-providers.md` 头部 v0.110→v0.110.2 + §1 表格 current defaultModel 列填实; `docs/polyrocket-llm-management.md` §15.7 placeholder → 真实表格 (5 providers × 5 cols),加 v1.3.1 changelog; `docs/overview.md` v2.64→v2.65 (this entry); `docs/coding-spec.md` v2.23→v2.24; new `docs/polyrocket-v0.110.2-final.md` ship log。**No new Rust, no new IPC, no codegen change**。**Coverage**: unchanged (codegen-irrelevant)。**Test count**: 1072→1077 (+5, all real)。**Research & sync mechanism** (per §15 SLA): 2-week hard SLA, Mavis cron checks vendor sites, when new stable version found, auto PR updates `LlmStep.tsx` + `§15.7` + `llm-providers.md` §1。详细见 `polyrocket-v0.110.2-final.md` ship log。
